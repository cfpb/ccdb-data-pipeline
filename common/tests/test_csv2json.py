...
class TestMain(unittest.TestCase):
    def setUp(self):
        ...
        self.optional = ["--heartbeat", "2", "--limit", "3"]
        self.positional = [fixtureToAbsolute("utf-8.csv"), self.actual_file]
    ...
    def test_json(self):
        self.optional.extend(["--json-format", "JSON"])
        argv = build_argv(self.optional, self.positional)
    ...
    def test_switch_fields(self):
        self.optional.extend(["--json-format", "JSON"])
        self.optional.extend(["--fields", fixtureToAbsolute("fields-good.txt")])
        argv = build_argv(self.optional, self.positional)

        with captured_output(argv) as (out, err):
            sut.main()

        validate_json(self.actual_file, fixtureToAbsolute("utf-8-switched.json"))
