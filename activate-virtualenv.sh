...
if [ -z $VENV_NAME ]; then
  export VENV_NAME="ccdb-data-pipeline"
fi
...
if ! which python3 &>/dev/null; then
  echo 'Error: python3 is not in your path.' \
       'Please ensure python3 is installed and available'
  return
fi
...
if workon $VENV_NAME; then
  ...
  if mkvirtualenv -p `which python3` $VENV_NAME; then
